...
def crawl_emails_and_phone_numbers(links):
    results = []
    for link in links:
        try:
            response = requests.get(link)
            if response.status_code == 200:
                emails = re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}(?:\.[A-Z|a-z]{2,})?\b', response.text)
                phone_numbers = re.findall(r'\b(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\b', response.text)
                ...
                results.append({
                    'link': link,
                    'error': 'Failed to retrieve content. Status code: {}'.format(response.status_code)
                })
...
async def extract_emails(session, url):
    ...
    payload = {"web_url": url, "info_request": {"emails": True,"phone_numbers": True}}
    results = []
    try:
        async with session.post(api_url, json=payload) as response:
            ...
            phone_number = data.get("meta_data", {}).get("phone_numbers",[])
            results.append({
                'emails': emails,
